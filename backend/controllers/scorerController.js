...
module.exports = {  // ✅ สำคัญ: ต้องมี module.exports ตรงนี้
    ...
    async saveMatchEvent(req, res) {
        try {
            ...
            switch (event_type) {
                case 'POINT':
                    skill = 'A'; // Enforce 'A' (Attack) for any point scored from console to pass skill_enum
                    grade = '#'; // '#' standard point
                    break;

                case 'SUBSTITUTION':
                    skill = 'S'; // Safe fallback string
                    grade = 'In'; 
                    break;

                case 'TIMEOUT':
                    skill = 'T'; // Or 'S' fallback
                    grade = 'Request';
                    break;
                ...
                case 'SANCTION':
                    skill = 'E'; // E for Error/Sanction
                    grade = details?.card || 'Yellow'; 
                    break;
