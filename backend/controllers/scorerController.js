    ...
    async endSet(req, res) {
        ...
        try {
            ...
            const updateMatchRes = await client.query(`
                UPDATE matches 
                SET home_sets_won = COALESCE(home_sets_won, 0) + $1,
                    away_sets_won = COALESCE(away_sets_won, 0) + $2,
                    current_set = current_set + 1
                WHERE id = $3
                RETURNING home_sets_won, away_sets_won, home_team_id, away_team_id
            `, [homeSetWin, awaySetWin, matchId]);

            const matchData = updateMatchRes.rows[0];
            const currentHomeSets = matchData.home_sets_won;
            const currentAwaySets = matchData.away_sets_won;

            // 4. ตรวจสอบเงื่อนไขจบการแข่งขัน (ชนะ 3 ใน 5 เซต)
            let isMatchFinished = false;
            let winnerId = null;

            if (currentHomeSets >= 3) {
                isMatchFinished = true;
                winnerId = matchData.home_team_id;
            } else if (currentAwaySets >= 3) {
                isMatchFinished = true;
